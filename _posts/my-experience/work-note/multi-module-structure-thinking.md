...
```
두번째 방식은 base package 의 확장입니다. Spring Boot는 아래와 같이 scan 되는 package를 확장할 수 있습니다.

@SpringBootApplication(scanBasePackages = "com.baemin.xxx.domain.shop")
public class XXXApplication {
이 방식이 스프링에서 권장하는 방식이며, 이 방식을 사용하는 것이 스프링의 의도와 잘 맞는 것 같습니다. 그러나 이 방식을 사용할 때 역시 저는 불편함을 느꼈습니다. 모듈 의존성을 추가하고 scanBasePackages를 추가로 작성하는 것이 번거롭다고 생각했습니다.
...
```
implementation 은 하위 의존을 숨겨주므로, 우리는 보호받아야할 하위 모듈에 대한 접근을 폐쇄할 수 있습니다.
그래서 저는 어플리케이션 모듈 계층 에서는 implementation 을 사용하고 그 외 계층에서는 api 방식을 사용하여 개발을 하였습니다.
```
...
| 관심사 | 정의                                                                | 코드 주요 변경 사유                      | 변경 상황 예시                                                               |
| ---- |-------------------------------------------------------------------|----------------------------------|------------------------------------------------------------------------|
| 도메인 | 도메인의 정체성(?)을 나타내는 핵심 코드들 => 외부 의존 필요없이 순수 자바 코드로만 표현 가능한 ?        | 도메인 요구사항 추가/변경 등                 | 수수료 계산 방식이 달라짐                                                         |
| 기능 구현 | 해당 도메인의 목표(ex : 정산 => 약속된 날짜에, 정확한 대상에게, 정확한 금액을 전달)를 달성하기 위한 코드들 | 기능/비기능적 요구사항                     | mail로만 알림하던걸 문자로도 알림 요구, 특정 기능 수행 후 이벤트 발행 추가, 조회 성능 개선을 위해 redis 도입 등 |
| 인프라 | 기능 구현, 시스템 운영 등을 위해 의존해야할 인프라스트럭쳐                                 | 인프라 변경, 설정 변경, 버전업, 인프라 의존성 추가 등 | mysql 5 -> mysql8 버전 업그레이드, 특정 데이터 저장을 위해 nosql 추가                     |
| 외부 연동 | 기능 구현, 시스템 운영 등을 위해 의존해야할 외부에서 구축한 시스템(특정 PG사, 결제 시스템 등)          | 외부 연동 시스템에 변경사항 발생               |                                                                        |

- 외부 연동 => 특정 PG사, 결제 시스템 등 ? => 결제도 여기서 다루는게 좋을까 ? => 외부 연동 시스템에 변경이 있을때 변경

| 구분        | 설명                                          |
| --------- | ------------------------------------------- |
| **인프라**   | 내가 관리하는 기술 자원 (DB, Redis, Kafka, 내부 공통 시스템) |
| **외부 연동** | 내가 통제할 수 없는 외부 플랫폼 또는 타사 시스템               |


| 관심사                             | 정의                                                        | 코드 주요 변경 사유                    | 변경 상황 예시                                                                               |
| ------------------------------- | --------------------------------------------------------- | ------------------------------ | -------------------------------------------------------------------------------------- |
| **도메인**                         | 비즈니스의 본질적 규칙을 나타내는 순수 로직. 외부 세계와 단절되어 구현 가능.              | 비즈니스 정책 변화, 규칙 추가/삭제           | - 수수료 규칙 변경<br>- 취소/반품 정책 변경<br>- 등급별 할인 정책 수정                                         |
| **기능 구현(Application / Flow)**   | 도메인 규칙을 조합해 실제 기능을 실행하는 계층. 배치/웹/이벤트 흐름 등.                | 기능 요구사항 변경, UX 변경, 새로운 Flow 추가 | - 기존 배치를 API에서도 실행 요청 필요<br>- Admin에서 수동 재처리 기능 추가<br>- 특정 시점마다 Alert 발송 로직 변경         |
| **인프라(Infrastructure)**         | 시스템 운영을 위해 필요한 기술적인 자원(DB, 캐시, 메시징 등). 내가 관리하거나 설정 가능한 것. | 기술 환경 변경, 설정 변경, 버전업, 성능 요구사항  | - MySQL 5 → 8 업그레이드<br>- Redis 클러스터링 적용<br>- Kafka 토픽 파티션 수 증가<br>- MyBatis에서 JPA로 전환  |
| **외부 연동(External Integration)** | 내가 운영하지 않는 외부 시스템과의 연결(PG사, 문자, 배송사 API 등).               | 외부 API 변경, 계약 변경, 결제 규정 변경     | - PG사 정산 API 스펙 변경<br>- 문자 발송 업체 교체<br>- 외부 인증 API 버전 업그레이드<br>- 배송사 API 장애로 재시도 정책 수정 |


- 도메인 변경 예시
  - 환불 가능한 기간 7일 → 14일로 변경
  - “승인 완료” 상태 없이 바로 “정산 요청” 가능하도록 정책 변경
  - 신규 “정책 요율” 추가

- 기능 구현 변경 예시
  - 관리자에서 특정 배치를 수동으로 실행 가능하게 요청
  - 기존 기능 완료 후 Slack 알림 발송 추가
  - 특정 조회 API를 캐싱 처리로 변경
  - API 응답 포맷 변경(페이징 구조 변경)

- 인프라 변경 예시
  - Redisson으로 분산 락 교체
  - Kafka consumer group 분리
  - Redis TTL 정책 변경
  - DB sharding 도입
  - MyBatis → QueryDSL 전환

- 외부 연동 변경 예시
  - PG사가 인증 토큰 기반 API로 스펙 전환
  - 문자 발송 업체의 callback URL 패턴 변경
  - 외부 정산 시스템의 응답 필드 변경
  - 외부 배송사 API 장애로 인해 fallback 정책 재정의
  - 외부 KYC 인증 요건 강화로 필수 필드 변경
...
### JPA 엔티티는 어디에 있는게 좋을까 ?


### 도메인 로직, 비즈니스 로직 등 생각해보기
=> 어떤 기준으로 잡아야할까 ?? 즉, 정확히 어떤 로직들이 core에 들어가고 어떤 로직들이 기능 구현의 관점일까 ,,
