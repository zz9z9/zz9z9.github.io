...
**한눈에 보기 — 각 락이 무엇을, 왜 막나**

행 수준 락은 결국 **인덱스라는 수직선 위에서 레코드(점)와 갭(점 사이 빈 구간)을 잠그는 일**이다. 아래 그림에서 각 락이 선의 어디를 잠그고, 그걸로 무슨 사고를 막는지 먼저 잡고 가면 개별 설명이 쉽게 들어온다.
...
───┤    ├──────[  20  ]──────┤    ├──────[  30  ]──────┤    ├───
   └────┘         ▲          └────┘         ▲          └────┘
      ▲           │             ▲           │
      │        이 행을            │        이 행을
   이 빈틈에    동시에 수정       이 빈틈에     동시에 수정
   새 행 삽입   (lost update)   새 행 삽입   (lost update)
   (phantom)                  (phantom)

└─ Gap Lock ─┘└Record Lock┘
└──────── Next-Key Lock ────────┘   (레코드 + 그 앞 갭을 한 덩어리로)
```

| 막고 싶은 문제 | 잠그는 대상 | 락 종류 | 어떻게 막나 |
| --- | --- | --- | --- |
| 이미 있는 행을 둘이 **동시에 수정** (lost update) | 레코드(점) | **Record Lock** | X는 단독 → 한 트랜잭션만 그 행을 쥔다 |
| 빈틈에 **새 행이 끼어듦** (phantom) | 갭(구간) | **Gap Lock** | 그 갭으로의 INSERT를 대기시킨다 |
| 위 둘을 **한꺼번에** (RR 기본 스캔) | 레코드 + 앞 갭 | **Next-Key Lock** | 레코드 락과 갭 락을 묶어서 건다 |
| INSERT끼리 **다른 자리인데도 서로 막힘** | 삽입할 갭 | **Insert Intention** | 위치가 다르면 서로 대기 안 함 |
| 행 락과 **테이블 락이 공존 가능한지** 빠른 판정 | 테이블 | **Intention (IS/IX)** | 테이블에 "행을 잠글 것"이라 미리 표시 |
| `AUTO_INCREMENT` **채번 충돌** | 테이블 | **AUTO-INC** | INSERT 동안 채번을 직렬화 |

- 위쪽 셋(Record / Gap / Next-Key)이 **행 락의 핵심**이고, 나머지는 이들을 **보조·특수화**한 것이다.
- Insert Intention은 갭 락의 변형, Intention(IS/IX)·AUTO-INC는 행 락을 관리하기 위한 **테이블 수준** 장치다.

**막지 못하면 실제로 이런 사고가 난다**

- **Record Lock 없이 동시 수정 (lost update)** — 재고 차감을 보자. 두 주문이 동시에 `stock = 10`을 읽고 각자 `10 - 1 = 9`를 저장하면, 2개가 팔렸는데 재고는 9로 남는다(8이어야 한다). X 레코드 락이 한쪽을 대기시켜 `10 → 9 → 8`로 직렬화한다.
...
- **Gap Lock 없이 새 행 삽입 (phantom)** — "그 시간대 예약 0건"을 확인하고 INSERT하는 회의실 예약에서, 두 트랜잭션이 동시에 0건을 보고 둘 다 삽입하면 **중복 예약**이 된다. 잠글 행이 0건이라 레코드 락은 무력하고, "0건이라는 사실"은 갭 락만 잠글 수 있다. (상세 케이스는 아래 [Next-Key Lock](#next-key-lock) 참고)

- **Next-Key로 막는 범위 집계 (phantom)** — 7월 매출 합계를 낸 뒤 그 값으로 정산 레코드를 만드는 사이 7월 주문이 하나 더 들어오면, 정산액과 실제 합계가 어긋난다. 넥스트키 락이 7월 범위 전체를 동결해 막는다.

- **Insert Intention 없이 INSERT 직렬화** — 서로 **다른 갭**에 삽입하는 INSERT끼리도 갭 락 하나 때문에 전부 줄 세워지면 INSERT 처리량이 급락한다. Insert Intention은 "삽입 위치가 다르면 서로 안 막는다"로 이걸 푼다.

- **Intention 락 없이 테이블 락 판정** — `LOCK TABLES ... WRITE`가 들어올 때, 테이블에 IX 표시가 없으면 **개별 행 락을 전부 뒤져야** 충돌 여부를 알 수 있다. 행이 수백만 개면 그만큼 느려진다. IX 한 건만 보고 즉시 충돌로 판정한다.

- **AUTO-INC 채번 충돌** — 동시 INSERT에서 채번이 안전하지 않으면 같은 `AUTO_INCREMENT` 값이 두 번 나가거나, statement 기반 복제에서 소스와 레플리카의 값이 어긋난다.
...
```sql
-- Tx A : (20,30) 갭을 잠금
SELECT * FROM t WHERE id BETWEEN 21 AND 30 FOR UPDATE;
-- 30에 넥스트키 → 갭 (20,30) [= 레코드 30에 매달림] + 레코드 30

-- Tx B : (10,20) 갭을 잠금
SELECT * FROM t WHERE id BETWEEN 11 AND 19 FOR UPDATE;
