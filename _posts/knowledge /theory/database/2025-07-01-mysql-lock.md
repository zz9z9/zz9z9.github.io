...
- **Next-Key로 막는 정원 제한 (phantom)** — "이 항공편 예약이 정원 미만"임을 `FOR UPDATE`로 세고 한 자리 넣는 사이, 다른 트랜잭션도 같은 값을 보고 넣으면 **초과 예약**이 된다. 넥스트키 락이 `flight_id` 범위를 동결해, 세는 동안 그 범위에 새 예약이 못 끼게 막는다.
...
**※ RC에서 "FK/중복 키 검사엔 갭이 남는다"를 실제로 찍어보면**

위 매뉴얼 문구를 `performance_schema.data_locks`로 확인해보면(MySQL 8.0.46), **중복 키 검사 쪽은 갭이 남는 게 명확히 재현되지만, FK 검사 쪽은 이 환경에서 재현되지 않았다.**
...
**※ 갭이 실제 인덱스 페이지에서 무엇인가**

"갭"을 값의 산술적 빈틈(`4`와 `5` 사이의 `4.5` 같은)으로 그리면 잘 안 잡힌다. 실제로는 **B+트리 리프 페이지의 정렬 링크드 리스트에서 "인접한 두 레코드 사이의 위치(삽입 자리)"** 다.

InnoDB PK 인덱스는 16KB 페이지 단위이고, 리프 페이지 안은 대략 이렇게 구성된다.

```
┌───────────────────────────────────────────────┐  ← 리프 페이지 (16KB)
│ File Header / Page Header                       │  prev·next 페이지 포인터, PAGE_N_RECS, LSN...
├───────────────────────────────────────────────┤
│ Infimum / Supremum (가상 레코드)                 │  heap_no=0, heap_no=1 로 고정
├───────────────────────────────────────────────┤
│ User Records (힙: "삽입 순서"대로 적재)           │  ← 실제 행. 키 순서가 아님
├───────────────────────────────────────────────┤
│ Free Space                                      │
├───────────────────────────────────────────────┤
│ Page Directory (이진탐색용 슬롯 배열)             │  4~8 레코드마다 슬롯 1개
├───────────────────────────────────────────────┤
│ File Trailer                                    │
└───────────────────────────────────────────────┘
```

핵심은 **레코드가 정렬돼 저장되는 게 아니라는 점**이다. 레코드는 힙에 **들어온 순서대로** 쌓이고, 키 정렬은 각 레코드 5바이트 헤더의 **`next_record`(다음 레코드까지의 상대 오프셋)** 포인터로만 표현된다. `10, 30, 20` 순으로 넣으면:

```
물리 힙 (삽입 순서)              논리 순서 (next_record 체인)
 h#0 [infimum]                  [infimum] ─▶ (10) ─▶ (20) ─▶ (30) ─▶ [supremum]
 h#2 (10)   ← 첫 삽입            h#0         h#2      h#4      h#3       h#1
 h#3 (30)   ← 둘째
 h#4 (20)   ← 셋째              물리적으론 20이 30보다 뒤에 있지만
 h#1 [supremum]                 체인은 10 ─▶ 20 ─▶ 30 으로 정렬돼 있다
```

`infimum`의 `next_record`는 항상 최소 키를, 최대 키의 `next_record`는 항상 `supremum`을 가리킨다. 그래서 **갭 `(4,5)` = "레코드 4의 `next_record`가 레코드 5를 가리키는 그 링크 구간"** 이다. 물리적 빈칸이 아니라 정렬 체인 위의 한 링크다.
...
핵심은 **"접근한 객체만 잠근다 + 조건에 매칭 안 되면 레코드 부분을 뺀다"** 이다.

- InnoDB는 범위 `[10,18]`을 추상적 구간으로 잠그지 않고 B+트리를 **레코드 하나씩 밟으며** 잠근다. 범위가 어디서 끝나는지 알려면 18 다음 레코드인 20을 실제로 밟아야 하므로, **20에도 락 엔트리가 생긴다.**
- 다만 20은 `id <= 18`에 **매칭되지 않는 경계 레코드**라, 레코드 부분은 빠지고 팬텀 방지에 필요한 **갭 `(10,20)`만** 남는다(`X,GAP`). 갭은 "오른쪽 레코드에 붙어" 저장되므로 20의 엔트리에 얹힐 뿐, 레코드 20 자체가 잠기는 건 아니다.
...
두 트랜잭션이 동시에 179석을 세면 둘 다 "정원 미달"로 보고 둘 다 INSERT → **181석, 초과 예약**이 된다. 넥스트키 락이 `flight_id = 7` 범위를 동결해, 한쪽이 세는 동안 다른 쪽 INSERT가 그 범위에 못 끼게 막는다.
...
| "활성 구독 없음"을 근거로 삽입 | **X** |
| 범위 집계(정원 미달)를 근거로 삽입 | **X** — 잠금 읽기 + 넥스트키 필요 |
